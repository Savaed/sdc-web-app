...
namespace SDCWebApp.Services
{
    ...
    public class CustomerDbService : ServiceBase, ICustomerDbService
    {
        ...
        public async Task<IEnumerable<Customer>> GetByAsync(Expression<Func<Customer, bool>> predicate)
        {
            ...
            try
            {
                var result = GetByPredicate<Customer>(predicate) as IEnumerable<Customer>;
            ...
            catch (Exception ex)
            {
                ...
                var internalException = new InternalDbServiceException($"Encountered problem when retrieving customers from the database using {nameof(predicate)}. See the inner excpetion for more details.", ex);
        ...
        /// <exception cref="InternalDbServiceException">The resource does not exist or has a null value or any
        /// other problems with retrieving data from database occurred.</exception>
        public async Task<IEnumerable<Customer>> GetWithPaginationAsync(int pageNumber = 1, int pageSize = 30)
        {
